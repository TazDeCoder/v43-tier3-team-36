...
export const queryCollectors = async (username: string, country: string) => prisma.user.findMany({
  where: {
    AND: [
...
export const deleteCollectionItem = async (comicId: number, userId: string) => prisma.collectionItem.delete({
  where: {
    comicId_userId: {
      comicId,
      userId,
...
export const getUserComic = async (userId: string, comicId: number) => prisma.collectionItem.findFirst({
  where: {
    userId,
    comicId,
  },
...
export const checkTradedOffer = async (id: string, userComic: any) => prisma.tradeOffer.findFirst({
  where: {
    createdBy: { id },
    AND: {
...
export const getTradeOfferByTradeOfferId = async (id: string, userId: string) => prisma.tradeOffer.findFirst({
  where: { id, createdBy: { id: userId } },
});

export const getTradeOffer = async (tradeOfferId: string) => prisma.tradeOffer.findUnique({
  where: { id: tradeOfferId },
});

export const deleteTradeOfferByTradeOfferId = async (tradeOfferId: string) => prisma.tradeOffer.delete({
  where: { id: tradeOfferId },
});
...
export const viewComicBookOffers = async () => prisma.tradeOffer.findMany({
  where: {
    status: 'PENDING',
  },
  include: { createdBy: true, collection: true },
...
export const queryTradeOffers = async (country: string) => prisma.tradeOffer.findMany({
  where: {
    status: 'PENDING',
    createdBy: {
      country: {
...
export const findTradeRequest = async (tradeRequestId: string) => prisma.tradeRequest.findUnique({
  where: {
    id: tradeRequestId,
  },
  include: {
...
export const storePushNotification = async (tradeOffer: any, receiver: any) => prisma.pushNotification.create({
  data: {
...
export const findPushNotification = async (id: string) => prisma.pushNotification.findMany({
  where: {
    userId: id,
  },
  orderBy: {
...
export const findNotification = async (id: string) => prisma.pushNotification.findMany({
  where: {
    id,
  },
});

export const updateNotification = async (notificationId: string) => prisma.pushNotification.update({
